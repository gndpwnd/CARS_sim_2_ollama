...
get_loaded_models() {
    # Get all ollama runner processes (these are models loaded in memory)
    ...
    # For each runner PID, try to find associated model from parent process
    for pid in $all_pids; do
        # Get parent PID
        local ppid=$(run_on_gpu "ps -o ppid= -p $pid 2>/dev/null | tr -d ' '")
        
        if [ -n "$ppid" ]; then
            # Get parent command line which should contain model name
            local cmdline=$(run_on_gpu "ps -o args= -p $ppid 2>/dev/null || true")
            
            # Extract model name if it's an ollama process
...
stop_all() {
    ...
    # Kill serve process (this will also kill loaded models)
    if [ -n "$serve_pid" ]; then
